...
- (void)searchEvents:(BOOL)withStatusBar {
    ...
    bookApi = [NSString stringWithFormat:@"%@%@?before=%@", BASE_URL, BOOK_SEARCH, currentDateStr];
...
- (void)searchEventsByKeyword:(NSString*)keyword{
    NSString *currentDateStr = [Functions convertDateToString:[NSDate date] format:@"yyyy-MM-dd HH:mm:ss"];
    bookFilterApi = [NSString stringWithFormat:@"%@%@?before=%@&keyword=%@", BASE_URL, BOOK_SEARCH, currentDateStr, keyword];
